    ...
    <div v-if="vacancy && Object.keys(vacancy).length">
      <q-dialog v-model="dialogApprove">
        <q-card style="width: 700px; max-width: 80vw;">
          ...
          >Rejeitar empresa {{ vacancy.title }}?</q-card-section>
          <q-card-section
            :class="getFont.text"
            v-else
          >Aprovar empresa {{ vacancy.title }}?</q-card-section>
          <q-card-actions align="right">
            <q-btn
              v-if="vacancy.approved"
...
export default {
  ...
  methods: {
    approveVacancy () {
      ...
      let vacancy = {}
      ref.doc(this.vacancy.id).get().then(doc => {
        if (doc.exists) {
          vacancy = doc.data()
          vacancy.approved = !vacancy.approved
          ref.doc(vm.vacancy.id).set(vacancy).then(function () {
            Loading.hide()
